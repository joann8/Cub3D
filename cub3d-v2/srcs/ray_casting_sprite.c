...
/*   Updated: 2021/02/02 19:38:49 by jacher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	sprite_ray(t_data *d, double x_h, double y_h)
{
	...
	//	printf("s.x = %i | s.y = %d \n",d->sprite[i].x_i, d->sprite[i].y_i);
		if (x == d->sprite[i].x_i && y == d->sprite[i].y_i && d->sprite[i].visible == 0)
		{
			...
			d->sprite[i].dist = calculate_distance(d->player->x, d->sprite[i].x_d, d->player->y, d->sprite[i].y_d);
			return (1);
		}
		i++;
	}
...
//	double temp_angle;	
	...
	angle_sprite_to_ortho = atan2(d->sprite[i].y_toplayer, d->sprite[i].x_toplayer);
	//printf("temp angle = %f\n", rad_to_deg(temp_angle));
	printf("player angle = %f\n", rad_to_deg(d->player->angle));
	printf("avant : angle sprite to ortho = %f\n", (angle_sprite_to_ortho));
	//if (angle_sprite_to_ortho < -M_PI)
	//	angle_sprite_to_ortho += 2 * M_PI;
//	if (angle_sprite_to_ortho >= M_PI)
//		angle_sprite_to_ortho -= 2 * M_PI;
//	printf("apres : angle sprite to ortho = %f\n", rad_to_deg(angle_sprite_to_ortho));
	angle_player_to_sprite = d->player->angle+ angle_sprite_to_ortho;
	//- d->player->angle;
	printf("avant : angle player to sprite = %f\n", rad_to_deg(angle_player_to_sprite));
	printf("    cos sprite angle = %f\n", angle_player_to_sprite);
	//angle_player_to_sprite = within_rad(d->player->angle - angle_sprite_to_ortho);
	//printf("    within rad cos sprite angle = %f\n", angle_player_to_sprite);
//	printf("apres : angle player to sprite = %f\n", rad_to_deg(angle_player_to_sprite));

	d->sprite[i].angle = angle_player_to_sprite;
			printf("1.cos sprite angle = %f\n", cos(d->sprite[i].angle));
	d->sprite[i].angle = within_rad(angle_player_to_sprite);
			printf("2.cos sprite angle = %f\n", cos(d->sprite[i].angle));
	...
	//		corrected_dist = d->sprite[i].dist * cos(d->ray->angle);
		...
		// d->sprite[i].height = (((d->map->tile_col / corrected_dist) * dist_proj_plan));
	d->sprite[i].height = ((dist_proj_plan / corrected_dist) * d->map->tile_col);
	d->sprite[i].length = ((dist_proj_plan / corrected_dist) * d->map->tile_lin);
	int mod;
	(void)mod;
