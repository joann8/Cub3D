...
/*   Updated: 2021/02/10 18:21:03 by jacher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		fill_file_header(t_data *d, t_file_header *fh)
{
	...
	img_calcul = (3 * d->map->r_y * d->map->r_x)
		+ d->map->r_y * get_byte_correction(d->map->r_x);
...
void	fill_img_header(t_img *img, t_file_header *fh, int fd, int x)
{
	t_bmp_color		pixel;
...
int		save_bmp(t_data *d)
{
	...
	{
		x = 0;
		fill_img_header(d->mlx->img, &fh, fd, x);
	}
	close(fd);
	if (fd == -1)
		return (print_error_else(1));
