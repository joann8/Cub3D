...
/*   Updated: 2021/02/11 22:35:08 by jacher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_cast_horizontal(t_data *d, t_coord *c, t_hit *h)
{
	...
	if (d->ray->fac_up == 1)
		min = 1;
	while ((c->x_next >= 0 && c->x_next < BLOCK * d->map->map_col)
			&& (c->y_next - min >= 0 && c->y_next < BLOCK * d->map->map_lin))
	{
...
void	ray_cast_vertical(t_data *d, t_coord *c, t_hit *h)
{
	...
	if (d->ray->fac_left == 1)
		min = 1;
	while ((c->x_next - min >= 0 && c->x_next < BLOCK * d->map->map_col)
			&& (c->y_next >= 0 && c->y_next < BLOCK * d->map->map_lin))
	{
